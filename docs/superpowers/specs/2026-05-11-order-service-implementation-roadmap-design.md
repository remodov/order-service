...
- **Ф2 — ГОТОВА** (план `docs/superpowers/plans/2026-05-12-phase2-persistence-outbox.md`; коммиты `feat(Ф2): Liquibase initial schema v-1.0 …`, `feat(Ф2): JooqOrderRepository …`, `feat(Ф2): Outbox-relay …`, `test(Ф2): интеграционный smoke …`). Liquibase-схема `migrations/db/changelog/v-1.0/` (9 таблиц `orders/order_items/outbox/processed_events/idempotency_keys/refund_sagas/order_summaries/order_timelines/order_audit_log` + enum-тип `order_status`; деньги `numeric(19,2)`, время `timestamptz`, jsonb для payload, FK с индексами, partial-index `idx_outbox_unpublished`, `Discount` = 3 колонки + CHECK; адрес доставки — `text`, шифрование at-rest = долг Ф7; check-constraints через raw `sql` т.к. `addCheckConstraint` — liquibase-commercial). `JooqOrderRepository` (реализует `OrderRepository`: `findById`/`findByIdForUpdate` (FOR UPDATE), `save` — upsert `orders`+`order_items` + append доменных событий в `outbox` с JSONB-payload + `clearDomainEvents`, `delete`; `@Transactional` на handler'е не на репозитории), `OrderDomainRecordMapper` (Plain Java: jOOQ-POJO ↔ domain — `Money`+currency, sealed `Discount`, `Address`, типизированные id, `Instant ↔ OffsetDateTime`, generated `OrderStatus` ↔ domain), `EventPayloadSerializer`. Outbox-relay: `ExternalEventPublisher` port в `core/`, `LoggingExternalEventPublisher` стаб в bootstrap (`@ConditionalOnMissingBean(name="kafkaExternalEventPublisher")`, BS-15), `OutboxRelay` `@Scheduled @Transactional` (`SELECT … FOR UPDATE SKIP LOCKED LIMIT batch` → publish → `UPDATE published_at`). Прошло `/ucp-pg-schema-review` (без MUST). `./gradlew build` зелёный, ArchUnit PASS, `bootRun` на `local` UP (Liquibase накатывает v-1.0). **Workflow сборки `adapter-out-postgres`:** jOOQ codegen из применённой Liquibase-схемы — `docker compose up -d postgres` → `./gradlew :adapter-out-postgres:generateJooq --no-configuration-cache` (или `regenerate` = `update`+`generateJooq`; liquibase Gradle plugin несовместим с configuration-cache) → `build`; generated-классы в `build/generated/jooq/` (gitignored). **Долги Ф2:** интеграционный `OrderPersistenceIntegrationTest` не прогоняется в текущем sandbox (форкнутый test-JVM не достучался до Docker-сокета) — SKIPPED через `@Testcontainers(disabledWithoutDocker=true)`, в CI/обычном dev выполняется; шифрование `Address` at-rest — Ф7; `OrderConfirmed` etc. payload поле `totalAmount`/`createdAt` vs spec-контракт `total`/`occurredAt` — сверить в Ф4 (`/ucp-api-design` / Kafka-контракты).
- **Ф3 — В РАБОТЕ** (план `docs/superpowers/plans/2026-05-12-phase3-happy-path-commands.md`, 10 задач). Сделано: Task 1 (pre-flight; `usecase-pattern` API: `UseCase<R>` маркер, `UseCaseCommand`/`UseCaseQuery` в `ru.vikulinva.usecase.cqrs`, `UseCaseHandler<U,R>` с `handle(U)`/`useCaseType()`, `UseCaseDispatcher.dispatch(UseCase<R>): R`); Task 2 (коммит `feat(Ф3): иерархия исключений + GlobalExceptionHandler …` — `AbstractCodedException` база, `OrderDomainException`/`IntegrationException`/`ValidationException`/`TechnicalException`, `GlobalExceptionHandler` `@RestControllerAdvice` → RFC 9457 ProblemDetail, маппинг code→HTTP по `13-errors`; `./gradlew :core:build :adapter-in-rest:compileJava` SUCCESSFUL). **Осталось:** Task 3 `/ucp-api-design` (OpenAPI yaml + DTO + `OrderRestMapper`), Task 4 `/ucp-integration-design` Catalog (`CatalogPort` + `CatalogClientAdapter` + Resilience4j + HealthIndicator), Task 5 `/ucp-pattern-design` `CreateOrder` + idempotency (`IdempotencyKeyRepository` port + `JooqIdempotencyKeyRepository`), Task 6 `AddItem`/`RemoveItem`/`ApplyPromo`/`RemovePromo`, Task 7 `ConfirmOrder` + `GetOrderById`, Task 8 `OrderController` wiring (после UseCase'ов!) + `/ucp-pattern-review`/`/ucp-api-review`, Task 9 `/ucp-test-design` интеграционные тесты, Task 10 финал.
- Ф4–Ф8 — не начаты.
